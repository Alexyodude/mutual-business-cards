...
// Brand tokens (Lab / mutual.solutions)
const BG_GRAD = 'linear-gradient(135deg,#0f172a 0%,#1e293b 100%)';
const ACCENT = '#22d3ee';   // cyan
const ACCENT_SOFT = '#4a8b9c'; // teal
const FG = '#fff';
const FG_DIM = '#94a3b8';
...
html,body{width:1050px;height:600px;background:#0f172a;font-family:'Inter','Segoe UI','SF Pro Display','Helvetica Neue',Helvetica,Arial,'Malgun Gothic','Apple SD Gothic Neo',sans-serif;color:#fff;overflow:hidden;-webkit-font-smoothing:antialiased}
.card{position:relative;width:1050px;height:600px;background:${BG_GRAD};overflow:hidden}
...
.tick{position:absolute;width:18px;height:18px;border-color:${ACCENT_SOFT}}
...
const EDGES = {
  ...
  capture: {
    ...
    decor: () => `
      ...
      <div style="position:absolute;left:32px;top:80px;width:3px;height:3px;background:${ACCENT};border-radius:50%;box-shadow:0 0 16px ${ACCENT}"></div>
      <div style="position:absolute;left:32px;bottom:80px;width:3px;height:3px;background:${ACCENT};border-radius:50%;box-shadow:0 0 16px ${ACCENT}"></div>`,
    // Single cyan ring centered on the card — camera-aperture / lens vibe.
    // Card center is x=525, y=300. Ring 240×240, stroke 2px.
    frontOverlay: () => `
      <div style="position:absolute;left:405px;top:180px;width:240px;height:240px;border:2px solid ${ACCENT};border-radius:50%;box-shadow:0 0 30px ${ACCENT}55, inset 0 0 30px ${ACCENT}22;z-index:1;pointer-events:none"></div>
      <div style="position:absolute;left:521px;top:296px;width:8px;height:8px;background:${ACCENT};border-radius:50%;box-shadow:0 0 12px ${ACCENT};z-index:1;pointer-events:none"></div>`,
  },
};
...
<div class="card">
  ...
  <div style="position:absolute;top:104px;left:${li}px;display:flex;align-items:center;gap:28px;z-index:2">
    <img src="logo-white.svg" style="width:104px;height:104px;object-fit:contain">
    <div style="font-weight:800;font-size:96px;letter-spacing:-3.8px;color:${FG};line-height:1">mutual<span style="font-size:24px;vertical-align:super;font-weight:500;letter-spacing:0;margin-left:4px;color:${FG_DIM}">™</span></div>
  </div>

  <!-- tagline (Korean primary, English secondary) -->
  <div style="position:absolute;left:${li}px;bottom:172px;text-align:left;z-index:2">
    <div class="ko" style="font-weight:700;font-size:36px;color:${ACCENT_SOFT};letter-spacing:-0.8px;line-height:1.2">진본은 출처에서 시작됩니다.</div>
    <div style="margin-top:14px;font-weight:600;font-size:30px;color:${FG_DIM};letter-spacing:-0.4px;line-height:1.3;font-style:italic">Authenticity starts at the source.</div>
  </div>

  <!-- divider -->
  <div style="position:absolute;left:${li}px;right:${ri}px;bottom:96px;height:1px;background:${FG_DIM}44"></div>

  <!-- meta row -->
  <div class="mono" style="position:absolute;left:${li}px;right:${ri}px;bottom:60px;display:flex;justify-content:space-between;align-items:center;font-size:24px;color:${FG_DIM};letter-spacing:0.6px;font-weight:500;z-index:2">
...
<div class="card">
  ...
  <div style="position:absolute;top:64px;right:${ri}px;display:flex;align-items:center;gap:14px;z-index:2">
    <img src="logo-white.svg" style="width:48px;height:48px;object-fit:contain">
    <div style="font-weight:800;font-size:32px;letter-spacing:-0.8px;color:${FG}">mutual<span style="font-size:13px;vertical-align:super;color:${FG_DIM};font-weight:500">™</span></div>
  </div>

  <!-- name block -->
  <div style="position:absolute;top:172px;left:${li}px;max-width:600px;z-index:2">
    <div style="font-weight:800;font-size:60px;letter-spacing:-1.8px;color:${FG};line-height:1">${c.name}</div>
    <div class="ko" style="margin-top:10px;font-weight:700;font-size:32px;color:${FG_DIM};letter-spacing:-0.4px">${c.nameKo}</div>
    <div style="margin-top:18px;font-weight:600;font-size:28px;color:${ACCENT};letter-spacing:-0.3px">${c.title}</div>
    <div class="ko" style="margin-top:6px;font-weight:500;font-size:22px;color:${FG_DIM}">${c.titleKo}</div>
    <div style="margin-top:26px;width:96px;height:3px;background:${ACCENT}"></div>
  </div>

  <!-- contact mono block -->
  <div class="mono" style="position:absolute;left:${li}px;bottom:64px;font-size:22px;line-height:1.85;color:${FG};letter-spacing:0.1px;font-weight:500;z-index:2">
    <div>${c.email}</div>
    <div>mutual.solutions</div>
    <div>${c.linkedin}</div>
  </div>
  ...
  <!-- QR + label -->
  <img src="${c.qr}" style="position:absolute;right:${ri}px;bottom:96px;width:160px;height:160px;background:#0f172a;padding:10px;border-radius:6px;box-shadow:0 0 0 1px ${ACCENT}33;z-index:2">
  <div class="mono" style="position:absolute;right:${ri}px;bottom:64px;font-size:14px;color:${FG_DIM};letter-spacing:1px;text-align:right;width:160px;z-index:2"><span style="color:${ACCENT}">SCAN</span> · ADD · <span class="ko">스캔</span></div>
</div>${TAIL}`;
}
